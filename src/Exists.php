...
class Exists implements Rule, ValidatorAwareRule
{
     ...
     */
    public function __construct($table, $column = 'id', ?\Closure $mapResult = null)
    {
        $this->queryClient = 1 === \func_num_args() || ((!\is_string($table) && \is_callable($table)) || \is_object($table)) ?
            $table : (static::isValidURL($table) ?
                new HTTPExistanceClient(rtrim($table ?? '', '/'), [], $mapResult) :
     ...
     *
     * @param string|\Closure|ExistanceVerifier|callable $table
     *
     * @return RulesExists|Exists
     */
    public static function create($table, ?string $key = 'id', ?\Closure $mapResult = null)
    {
        if (\is_string($table) && self::isValidURL($table)) {
            return new static(
                new HTTPExistanceClient($table, [], $mapResult),
                $key
            );
        }
        ...
        if (null !== $queryClient) {
            return new static($queryClient, $key);
        }
    ...
    {
        return 'The :attribute does\'nt exist.';
    }
